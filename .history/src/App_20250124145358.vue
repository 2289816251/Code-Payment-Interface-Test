<template>
  ...
  <div
    ...
    v-if="formState.commodity != '' && formState.price != 0"
  >
    <div
      class="item"
      @click="switchPaymentMethods(item)"
  ...
  <div class="paymentBtn" v-if="formState.commodity != '' && formState.price != 0">
    <a-button size="large" type="primary" :icon="h(PayCircleOutlined)">去支付</a-button>
  </div>
</template>
...
// 切换支付方式
const switchPaymentMethods = function (item) {
...
.paymentMethods {
  ...
  .item {
    ...
    &.active {
      ...
      padding: var(8px - 4px) var(16px - 4px);
    }
  }
}
.paymentBtn{
