...
export const useHomeStore = defineStore('home', {
  state: () => {
    return {
      unifiedOrderPaymentData: {
        ...
        type: '', // 支付方式
        out_trade_no: '', // 商户订单号
        notify_url: 'https://www.staruniverse.top/notify_url', // 异步通知地址
        return_url: 'https://simple-node-service-silk.vercel.app/return_url', // 跳转通知地址
        name: '', // 商品名称
        money: '', // 商品金额
  ...
  actions: {
    ...
    async paymentViaAPI(data) {
      ...
      console.log(data)
      try {
        let result = await api.getUnifiedOrderPayment(data)
        console.log(result)
        // 本地存储订单
