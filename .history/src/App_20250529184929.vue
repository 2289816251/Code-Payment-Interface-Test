<template>
  ...
  <!-- 扫码支付弹窗 -->
  <!-- <OrderPaymentModal :orderdata="ordersListData" :paymentInfo="paymentInfo()"/> -->
</template>
...
// 订单数据
let ordersListData = getOrdersList()
// 订单支付信息
let paymentInfo = ()=>{
    let data = homeStore.paymentMethods.filter((item,index)=>{
      console.log(item.type.includes(ordersListData.type))
        return item.type.includes(ordersListData.type)
    })[0]
    return data
}
