<template>
  ...
  <!-- 扫码支付弹窗 -->
  <OrderPaymentModal v-if="homeStore.payModelIsShow" :open="homeStore.payModelIsShow" :orderdata="ordersListData" :paymentInfo="paymentInfo()"/>
</template>
...
import { getOrdersList } from "@/hooks/useLocalStorageOrders";
...
// 订单数据
let ordersListData = getOrdersList()[0];
// 订单支付信息
let paymentInfo = () => {
  let data = homeStore.paymentMethods.filter((item, index) => {
    return item.type == ordersListData.type;
  })[0];
  return data;
};
// 减数量
...
onMounted(() => {
  ...
  // 获取支付渠道列表
  homeStore.getObtainPaymentChannelList();
});
</script>
